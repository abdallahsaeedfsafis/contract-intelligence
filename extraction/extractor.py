...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "- Free-text field values (parties' names, payment_terms, governing_law, termination_clauses, "
        "penalty_clauses, language_precedence_clause, duration) MUST be written in the SAME language "
        "as the text you were given - never translate or switch language, even partially. If the "
        "source text is in Arabic, every one of these values must be in Arabic, and likewise for "
        "English source text. This applies even when the source text is noisy, partially illegible, "
        "or garbled (e.g. from OCR on a scanned document) - do the best you can to transcribe or "
        "closely paraphrase what the garbled text most likely says, IN THAT SAME LANGUAGE. Never "
        "fall back to summarizing or explaining the content in a different language just because the "
        "source is hard to read; an approximate answer in the correct language is required, not a "
        "clean answer in the wrong one.\n"
