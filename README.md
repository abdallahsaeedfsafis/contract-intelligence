...
- **OCR is supported for scanned PDFs** (`extraction/parser.py` falls back to Tesseract with combined Arabic+English recognition when a PDF has little to no embedded text layer), but accuracy depends on scan quality, and OCR introduces more character-level errors than native text extraction - especially for Arabic. The UI flags OCR-derived contracts so this can be verified.
