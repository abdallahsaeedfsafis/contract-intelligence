...
- **Not yet tested on scanned/OCR'd contracts.** `extraction/parser.py` extracts text from `.txt`, `.docx`, and `.pdf` uploads, but a scanned PDF with no underlying text layer would need an OCR step first, which isn't implemented.
