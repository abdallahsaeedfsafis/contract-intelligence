...
- **OCR support** — scanned/image-based PDFs are automatically detected and processed via Tesseract OCR (Arabic + English), with a visible "verify accuracy" indicator since OCR introduces more errors than native text extraction.
...
### OCR text quality broke an assumption extraction had always gotten for free

With clean source text, the extractor naturally mirrors the source section's language - it was never explicitly instructed to, it just had no reason not to. A real scanned contract's OCR output was garbled enough to break that assumption: unable to confidently transcribe or paraphrase the barely-legible Arabic, the model fell back to a fluent English paraphrase of what it understood instead. The fix was the same shape as the contract_06 fix above - make an implicit assumption explicit - by adding a rule to the extraction prompt requiring free-text fields to stay in the source language even when that source is noisy or partially illegible.
...
- **OCR accuracy depends heavily on scan quality.** On a deliberately realistic test scan (rotation, blur, noise), OCR correctly extracted the seeded financial discrepancy but also introduced some genuine misreadings (e.g. a garbled effective_date) that the system correctly reported as-is rather than silently correcting - this is why the OCR badge exists and why OCR results should always be spot-checked against the source document.
- **PDF text extraction quality depends on how the source PDF was created.** PDFs generated by standard tools (Word export, print-to-PDF) extract correctly, but some programmatically-generated PDFs may have RTL character-ordering issues in the underlying text layer, which pypdf reproduces as-is.
